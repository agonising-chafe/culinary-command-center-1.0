  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl max-w-md w-full p-6 overflow-y-auto max-h-[90vh]">
        ...
        <p className="text-gray-500 mb-4">{selected.cookTime} · {selected.calories} kcal</p>
        <h3 className="font-semibold">Ingredients</h3>
        <ul className="list-disc list-inside mb-4 text-sm">
          {selected.ingredients.map((i) => <li key={i}>{i}</li>)}
        </ul>
        <h3 className="font-semibold">Instructions</h3>
        <ol className="list-decimal list-inside text-sm space-y-1">
          {selected.instructions.map((i) => <li key={i}>{i}</li>)}
        </ol>
